"""
This script produces the plots of the high multiplicity analysis.
"""
import numpy as np
...
show_ts_ibd = True #time serie of IBD candidates
show_ts_acc = True #time serie of acc candidates
...
for i in range(NA_DS20_Data.indices_off.size):
    acc_off[i] = NA_DS20_Data.acc_highMult[NA_DS20_Data.indices_off[i]] 
    ibd_off[i] = NA_DS20_Data.ibd_highMult[NA_DS20_Data.indices_off[i]] 

for i in range(NA_DS20_Data.indices_on.size):
    acc_on[i] = NA_DS20_Data.acc_highMult[NA_DS20_Data.indices_on[i]] 
    ibd_on[i] = NA_DS20_Data.ibd_highMult[NA_DS20_Data.indices_on[i]] 
...
if show_ts_ibd:
    ...
    plt.scatter(NA_DS20_Data.datas_media_intervalo, NA_DS20_Data.ibd_highMult, color='black', label = 'Candidates IBD')
    plt.plot(NA_DS20_Data.datas_media_intervalo, NA_DS20_Data.ibd_highMult, linestyle='--', color='black',alpha = 0.8)

    plt.xlabel('Dates')
    plt.ylabel('Event Number')
    plt.title('Neutrinos Angra - 2020 Dataset - IBD Candidates - High Multiplicity Analysis')
...
if show_ts_acc:
    ...
    plt.scatter(NA_DS20_Data.datas_media_intervalo, NA_DS20_Data.acc_highMult, color='black', label = 'Accidentals')
    plt.plot(NA_DS20_Data.datas_media_intervalo, NA_DS20_Data.acc_highMult, linestyle='--', color='black',alpha = 0.8)

    plt.xlabel('Dates')
    plt.ylabel('Event Number')
    plt.title('Neutrinos Angra - 2020 Dataset - Accidentals - High Multiplicity Analysis')
    plt.xticks(ticks=NA_DS20_Data.datas_selecionadas, labels=[data.strftime('%d/%m/%Y') for data in NA_DS20_Data.datas_selecionadas], rotation=60)
    plt.grid(axis='x')
...
if show_scatt_ibd_acc:
    x = np.array(np.take(NA_DS20_Data.acc_highMult,NA_DS20_Data.indices_off))
    y = np.array(np.take(NA_DS20_Data.ibd_highMult,NA_DS20_Data.indices_off))
    x_err = np.sqrt(x)
    y_err = np.sqrt(y)
    print(x)
    print(y)
    ...
    plt.scatter( np.take(NA_DS20_Data.acc_highMult,NA_DS20_Data.indices_off), np.take(NA_DS20_Data.ibd_highMult,NA_DS20_Data.indices_off), color='black', label = 'Reactor off')
    plt.plot(x_fit, y_fit, 'r-', label='linear LS fit')
    plt.xlabel('Accidentals')
    plt.ylabel('IBD')
    plt.title('Neutrinos Angra - 2020 Dataset - IBD Vs Accidentals - High Multiplicity Analysis')
    plt.grid(axis='x')
    plt.grid(axis='y')
    plt.legend()
    plt.tight_layout()
